...
def main(args):
    ...
    while True:
        frame_num += 1
        print(f'[INFO] frame number: {frame_num}')
        origin_img = camera.read()

        if not os.path.exists(args['frame_save_path']):
            os.makedirs(args['frame_save_path'])
        video_writer.write(origin_img)

        # if key == ord('s'):
        #     if not os.path.exists(args['frame_save_path']):
        #         os.makedirs(args['frame_save_path'])
        #     cv2.imwrite(os.path.join(args['frame_save_path'], str(frame_num) + '.jpg'), origin_img)
        ...
        # cv2.imshow('RTSP origin frame', origin_img)
        cv2.imshow('RTSP target frame', origin_img)
        # cv2.imshow('RTSP processed frame', processed_img.cpu().detach().numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--frame_save_path', default=os.path.join(c_folder, 'frame_save', '20200903'))
